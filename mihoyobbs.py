...
class mihoyobbs:
    ...
    def Get_taskslist(self):
        ...
        req = http.get(url=setting.bbs_Taskslist,
    ...
    def Getlist(self) -> list:
        ...
        req = http.get(url=setting.bbs_Listurl.format(
            setting.mihoyobbs_List_Use[0]["forumId"]), headers=self.headers)
        data = req.json()
        for n in range(5):
            temp_List.append([data["data"]["list"][n]["post"]["post_id"],
    ...
    def Signin(self):
        if self.Task_do["bbs_Sign"]:
            ...
            for i in setting.mihoyobbs_List_Use:
                req = http.post(url=setting.bbs_Signurl.format(
                    i["id"]), data="", headers=self.headers)
                data = req.json()
    ...
    def Readposts(self):
        if self.Task_do["bbs_Read_posts"]:
            ...
            for i in range(self.Task_do["bbs_Read_posts_num"]):
                req = http.get(url=setting.bbs_Detailurl.format(
                    self.postsList[i][0]), headers=self.headers)
                data = req.json()
                if data["message"] == "OK":
    ...
    def Share(self):
        if self.Task_do["bbs_Share"]:
            ...
            tools.log.info("正在分享......")
            req = http.get(url=setting.bbs_Shareurl.format(
